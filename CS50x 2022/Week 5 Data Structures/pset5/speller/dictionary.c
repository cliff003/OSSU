...
{
    // Use all letters to generate hash number (Best function, faster than spell50)
    int number = 0;
    for (int i = 0; i < LENGTH + 1; i++)
    {
        ...
        // Each letter multiply a coefficient in descending order
        number += toupper(word[i]) * (LENGTH - i);
    }
    return number;
